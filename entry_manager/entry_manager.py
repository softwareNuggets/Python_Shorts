#entry_manager.py
#written by software nuggets (c) 2024
#https://www.youtube.com/c/softwareNuggets
#https://www.github.com/softwareNuggets/python_shorts

import tkinter as tk
from tkinter import ttk

class EntryManager(ttk.Frame):
    def __init__(self, parent, section_title, entry_label_text, item_list=None, *args, **kwargs):
    ...
    def create_widgets(self):
        # Section title
        self.section_title_label = ttk.Label(self, text=self.section_title, font=('Arial', 14, 'bold'))
        self.section_title_label.grid(row=0, column=0, columnspan=2, pady=5, sticky='w')

        # Frame for entry display
        self.entry_frame = ttk.Frame(self)
        self.entry_frame.grid(row=1, column=0, columnspan=6, pady=5, sticky='w')

        # Entry Label and Entry
        self.entry_label = ttk.Label(self.entry_frame, text=self.entry_label_text, font=('Arial', 12))
        ...
        self.entry_box.grid(row=0, column=1, padx=(5, 0), pady=5, sticky='w')

        # Buttons
        self.add_button = ttk.Button(self.entry_frame, text="Add", command=self.add_entry)
        self.add_button.grid(row=0, column=2, padx=5, pady=5, sticky='w')
        ...
        self.update_button.grid(row=0, column=4, padx=5, pady=5, sticky='w')
        self.update_button.grid_remove()

        self.delete_button = ttk.Button(self.entry_frame, text="Delete", command=self.delete_entry)
        self.delete_button.grid(row=0, column=5, padx=5, pady=5, sticky='w')
        self.delete_button.grid_remove()

        # Listbox to display entries
        self.entry_listbox = tk.Listbox(self.entry_frame, height=6, width=50)
        self.entry_listbox.grid(row=1, column=1, columnspan=4, padx=5, pady=5, sticky='w')
